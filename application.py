...
def index():
    ...
    # Get and store the symbol and quantity of all stocks owned
    quantityrow = db.execute("SELECT symbol, SUM(quantity) quantity FROM transactions WHERE user = :currentuser GROUP BY symbol",
    ...
    for row in quantityrow:
        ...
        # Only add the stock if the quantity is greater than 0
        if quantity > 0:
...
def sell():
    ...
    # User reached route via GET
        ...
        # Get all the symbols for the user
        currentuser = session["user_id"]
        symbols = db.execute("SELECT symbol, sum(quantity) quantity FROM transactions WHERE user = :currentuser GROUP BY symbol",
                             currentuser=currentuser)

        # Find all valid symbols where the total quantity is greater than 0
        validsymbols = []
        for current in symbols:
            if current.get("quantity") > 0:
