...
def index():
    ...
    quantityrow = db.execute("SELECT symbol, SUM(quantity) quantity FROM transactions WHERE user = :currentuser GROUP BY symbol HAVING SUM(quantity) > 0",
...
def register():
    ...
    if request.method == "POST":
        ...
        # User has been created, redirect to main page and login
...
def sell():
    ...
    # User reached route via GET
        ...
        currentuser = session["user_id"]
        symbols = db.execute("SELECT symbol, SUM(quantity) quantity FROM transactions WHERE user = :currentuser GROUP BY symbol HAVING SUM(quantity) > 0",
                             currentuser=currentuser)
